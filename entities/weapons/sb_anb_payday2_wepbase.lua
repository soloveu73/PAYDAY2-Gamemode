...
SWEP.sb_anb_payday2_wepbase = true

if SERVER then
	util.AddNetworkString("sb_anb_payday2_wepbase_muzzle")
end
...
function SWEP:PrimaryAttack()
	...
	if SERVER and self.MuzzleEffect then
		net.Start("sb_anb_payday2_wepbase_muzzle",true)
...
hook.Add("PlayerCanPickupWeapon","sb_anb_payday2_wepbase",function(ply,wep)
	if wep.sb_anb_payday2_wepbase then return false end
end)

if CLIENT then
	net.Receive("sb_anb_payday2_wepbase_muzzle",function(len)
		local wep = net.ReadEntity()
		if !IsValid(wep) or !wep.sb_anb_payday2_wepbase then return end
